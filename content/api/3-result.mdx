...
_note: this does not reflect the number of rows __returned__ from a query.  e.g. an update statement could update many rows (so high `result.rowCount` value) but `result.rows.lenght` would be zero.  To check for an empty query reponse on a `SELECT` query use `result.rows.length === 0`_.
